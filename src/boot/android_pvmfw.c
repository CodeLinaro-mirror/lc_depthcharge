...
static int create_fw_android_node(DeviceTree *ref_dtb, char *bootconfig)
{
	...
	node = dt_find_node(ref_dtb->root, (const char **)fw_android_path, NULL, NULL, 1);
	if (!node)
		return -1;
...
static DeviceTree *create_avf_vm_ref_dt(const VbSelectAndLoadKernelParams *kparams)
{
	DeviceTree *ref_dtb;

	/* Create new empty device tree for the VM reference DT */
	ref_dtb = fdt_unflatten((void *)empty_dtb);
	if (!ref_dtb)
		return NULL;
 ...
 * }
 */
static int parse_boot_params_v0(const void *blob, size_t size, DeviceTree *vm_ref_dt,
...
static int parse_boot_params_v1(const void *cbor, size_t cbor_size,
				DeviceTree *vm_ref_dt, struct pvmfw_config_v1_3 *cfg,
				size_t max_cfg_size)
{
	int ret;
	ssize_t consumed;
 ...
 */
static int parse_boot_params(const void *blob, size_t size, DeviceTree *vm_ref_dt,
			     struct pvmfw_config_v1_3 *cfg, size_t max_cfg_size)
{
	/*
	 * Verify that the blob is at least big enough to compare the
...
int setup_android_pvmfw(const VbSelectAndLoadKernelParams *kparams, size_t *pvmfw_size,
			const void *params, size_t params_size)
{
	int ret;
	uintptr_t pvmfw_addr, pvmfw_max, cfg_addr;
	struct pvmfw_config_v1_3 *cfg;
	struct DeviceTree *vm_ref_dt;
