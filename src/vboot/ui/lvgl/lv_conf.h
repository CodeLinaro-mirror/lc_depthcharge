...
#ifndef VBOOT_UI_LVGL_LV_CONF_H
#define VBOOT_UI_LVGL_LV_CONF_H

/*
 * lvgl/src/lv_conf_internal.h prints warnings without LV_CONF_H defined.
 * Define LV_CONF_H here to indicate config file is imported correctly.
 */
#define LV_CONF_H
...
#define LV_STDINT_INCLUDE	"vboot/ui/lvgl/stdint_fixup.h"
...
#define LV_STDARG_INCLUDE	"stdarg.h"
	...
	/*
	 * Instead of an address give a memory allocator that will be called
	 * to get a memory pool for LVGL. E.g. my_malloc
	 */
	#if LV_MEM_ADR == 0
...
/*
 * Default Dots Per Inch. Used to initialize default sizes such as widgets sized, style
 * paddings. (Not so important, you can adjust it to modify default sizes and spaces.)
 */
...
#define LV_USE_OS   LV_OS_NONE
	 ...
	 * RTOS task notifications can only be used when there is only one task that can be the
 ...
 * it is buffered into a "simple" layer before rendering. The widget can be buffered in smaller
 * chunks. "Transformed layers" (if `transform_angle/zoom` are set) use larger buffers
 ...
 * It should be at least `LV_DRAW_LAYER_SIMPLE_BUF_SIZE` sized but if transformed layers are
 * also used it should be enough to store the largest widget too (width x height x 4 area).
...
#define LV_USE_DRAW_SW 1
	...
	/**
	 * - 0: Use a simple renderer capable of drawing only simple rectangles with gradient,
	 *	  images, text, and straight lines only.
	 * - 1: Use a complex renderer capable of drawing rounded corners, shadow, skew lines,
	 *	  and arcs too.
	 */
	#define LV_DRAW_SW_COMPLEX		  1
		 ...
		 *  LV_DRAW_SW_SHADOW_CACHE_SIZE is the maximum shadow size to buffer,
		 *  where shadow size is `shadow_width + radius`. Caching has
		 *  LV_DRAW_SW_SHADOW_CACHE_SIZE^2 RAM cost. */
     ...
     *  linear at an angle, radial or conical */
...
#define LV_USE_DRAW_VGLITE 0
	...
	/** Enable blit quality degradation workaround recommended for screen's
	 *  dimension > 352 pixels. */
	#define LV_USE_VGLITE_BLIT_SPLIT 0
		...
		#define LV_USE_VGLITE_DRAW_THREAD 1
			...
			/** Enable VGLite draw async. Queue multiple tasks and flash
			 *  them once to the GPU. */
...
#define LV_USE_LOG 0
	 ...
	 *  E.g `my_print`. The prototype should be
    ...
    /** Enable/disable trace logs in object creation (core `obj` creation + every widget). */
...
/** Default number of image header cache entries. The cache is used to store the headers of
 *  images. The main logic is like `LV_CACHE_DEF_SIZE` but for image headers. */
...
* - lv_obj_assign_id:    Called when a widget is created. Use a separate counter for each
*                        widget class as an ID.
 ...
 *  @note  vglite_src_buf_aligned() uses this value to validate alignment of passed buffer
 *         pointers. */
#define LV_ATTRIBUTE_MEM_ALIGN_SIZE 1

/** Will be added where memory needs to be aligned (with -Os data might not be aligned to
 *  boundary by default). E.g. __attribute__((aligned(4)))*/
...
/** Export integer constant to binding. This macro is used with constants in the form of
 *  LV_<CONST> that should also appear on LVGL binding API such as MicroPython. */
 ...
 *  - lv_dropdown_t	:  Options set to "Option 1", "Option 2", "Option 3",
 *                     else no values are set.
 *  - lv_roller_t     :  Options set to "Option 1", "Option 2", "Option 3", "Option 4",
 *                       "Option 5", else no values are set.
...
#define LV_USE_SYSMON   0
	...
	#define LV_USE_PERF_MONITOR 0
		 ...
		 *  1: Prints performance data using log. */
...
#define LV_USE_IME_PINYIN 0
	 ...
	 *  @note  If you do not use the default thesaurus, be sure to use `lv_ime_pinyin` after
...
#define LV_USE_X11			  0
#if LV_USE_X11
    /**< Exit application when all X11 windows have been closed */
	#define LV_X11_DIRECT_EXIT		 1
    /**< Use double buffers for rendering */
	#define LV_X11_DOUBLE_BUFFER	   1
	/* Select only 1 of the following render modes
     * (LV_X11_RENDER_MODE_PARTIAL preferred!).
     */
...
#define LV_USE_LIBINPUT	0
	...
	#define LV_LIBINPUT_XKB			 0
		...
		#define LV_LIBINPUT_XKB_KEY_MAP {\
			.rules = NULL, .model = "pc101", .layout = "us",\
...
#define LV_USE_DEMO_EBIKE			0
	...
	/*0: for 480x270..480x320, 1: for 480x800..720x1280*/
...
/*--END OF VBOOT_UI_LVGL_LV_CONF_H--*/

#endif /*VBOOT_UI_LVGL_LV_CONF_H*/
