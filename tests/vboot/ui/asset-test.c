...
/* Test functions */
static void test_ui_get_asset(void **state)
{
	const struct {
		char *name;
		int focused;
	...
	for (i = 0; i < ARRAY_SIZE(cases); i++) {
		...
		expect_string(ui_load_asset, file, cases[i].expected_file);
		ASSERT_VB2_SUCCESS(ui_get_asset(cases[i].name, NULL,
...
int main(void)
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(test_ui_get_asset),
	};
